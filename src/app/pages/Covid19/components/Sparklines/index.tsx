...
import { renderDate, renderTimestampDate } from '@utils/date/covid';
...
export default function Sparklines({ data: { monthly, daily } }): JSX.Element {
  ...
  const todayRecord = daily[daily.length - 1] || daily[daily.length - 2] || daily[daily.length - 3];
  return (
    <div>
      <div className="cards-container">
        ...
        <MiniChartCard
          ...
          chartData={monthly.map(({ onRespiratorForDate }) => onRespiratorForDate).join(', ')}
          seriesOptions={{
            color: '#fd7e14',
          }}
          ...
          growth={{
            value: todayRecord.percentOnRespiratorComparedWithHospitalizedForDate + '%',
        ...
        <MiniChartCard
          ...
          growth={{
            value: todayRecord.percentOfHospitalizedComparedWithInfectedSumForDate + '%',
